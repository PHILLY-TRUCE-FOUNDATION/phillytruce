...
class GetChatListByIdListClient {
  final _baseUrl = '${AddMediatorApiClient.baseUrl}';
  http.Client httpClient = http.Client();

  Future<ChatModel> getChatUserByIdList(userIdList) async {
    ...
    bodyParameter['user_id_list'] = jsonEncode(userIdList);
    ...
    //     '/' +
    //     userData.unique_token.toString());
    final url = '$_baseUrl/get_user_list_by_array';
    var responseJson;
    try {
  ...
  dynamic _response(http.Response response) {
    switch (response.statusCode) {
      case 200:
        print('imowef' + response.body.toString());
